...
if (isset($_POST['id'])) {
    ...
    if (mysqli_num_rows($result) > 0) {
        if ($id == $_SESSION['id']) {
            header("Location: ../admin/usuarios_admin.php?message=No puedes eliminar tu propio usuario");
            exit();
        }
        ...
        if (mysqli_query($conexion, $delete_sql)) {
            header("Location: ../admin/usuarios_admin.php?message=Usuario eliminado con éxito");
            exit();
        } else {
